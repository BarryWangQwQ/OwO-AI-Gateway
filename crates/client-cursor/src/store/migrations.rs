...
pub(super) async fn run(pool: &SqlitePool, database_path: &Path) -> Result<()> {
    tracing::debug!("running database migrations with diagnostics");
    ...
    tracing::debug!(known = ?known, "embedded database migrations");
    if applied.is_empty() {
        tracing::debug!("no applied database migrations were found");
    } else {
    ...
    tracing::debug!(pending = ?pending, pending_count = pending.len(), "pending database migrations");
    ...
    loop {
        tokio::select! {
            result = &mut future => {
                ...
                return match result {
                    Ok(()) => {
                        tracing::debug!(stage, elapsed_ms, "database migration stage completed");
                        Ok(())
                    }
                    Err(error) => {
...
async fn load_applied_migrations(pool: &SqlitePool) -> Result<Vec<AppliedMigration>> {
    tracing::debug!("reading database migration history");
    ...
    .await?;
    if table_exists == 0 {
        tracing::debug!("database migration history table does not exist");
        return Ok(Vec::new());
    }
    ...
    .fetch_all(pool)
    .await?;
    tracing::debug!(row_count = rows.len(), "database migration history loaded");
    rows.into_iter()
        .map(|row| {
            Ok(AppliedMigration {
...
fn log_file(kind: &str, path: &Path) {
    match std::fs::metadata(path) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => tracing::debug!(
            kind,
            path = %path.display(),
            exists = false,
            "SQLite file state"

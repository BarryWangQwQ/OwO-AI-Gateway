...
const DATA_DIR_NAME: &str = ".owo/state/cursor";
const DATABASE_FILE_NAME: &str = "cursor.db";
...
/// Places all Cursor integration state (database, CA, caches) under OwO AI Gateway's state
/// directory. Must be called before anything else touches the data directory.
pub fn set_managed_data_dir(dir: PathBuf) {
...
impl Config {
    ...
    /// Configuration for the backend embedded in `owo start`.
    pub fn for_owo(listen_addr: SocketAddr) -> Result<Self> {
        Ok(Self {
            listen_addr,

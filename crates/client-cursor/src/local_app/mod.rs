...
pub use self::ca::{TrustCommand, CA_COMMON_NAME};
pub use self::settings::settings_path as cursor_settings_path;
...
impl CursorHarness {
    ...
    /// The local CA certificate the OS must trust for the takeover to work.
    pub fn ca_certificate_path(&self) -> std::path::PathBuf {
        self.inner.ca.cert_file()
    }

    /// Privileged commands that add the CA to the OS trust store.
    pub fn ca_trust_commands(&self) -> Vec<TrustCommand> {
        self.inner.ca.trust_commands()
    }

    /// Privileged commands that remove the CA from the OS trust store.
    pub fn ca_untrust_commands(&self) -> Vec<TrustCommand> {
        self.inner.ca.untrust_commands()
    }

    /// Whether the OS trust store currently holds the CA (without touching the proxy).
    ...
    async fn enable(&self) -> Result<()> {
        ...
        // Cursor picks up a new `http.proxy` on its next start; the user restarts it.
        if proxy.running() {

...
pub async fn available_models(
    ...
    tracing::info!(model_count = models.len(), "appending OwO AI Gateway models to Cursor AvailableModels");
    let local = AvailableModelsAddition {
        model_names: models.iter().map(|model| model.model_hash.clone()).collect(),
...
pub async fn usable_models(
    ...
    let models = registry.store().models().await?;
    tracing::info!(model_count = models.len(), "appending OwO AI Gateway models to Cursor GetUsableModels");
    let local = UsableModelsAddition { models: models.iter().map(usable_model).collect() }
    .encode_to_vec();
    match proxy::forward_buffered(&proxy, request).await {
        Ok(upstream) => merge_response(upstream, local),
        Err(error) => {
...
pub async fn default_model_for_cli(
    ...
    let models = registry.store().models().await?;
    Ok(local_response(
        agent::GetDefaultModelForCliResponse { model: models.first().map(usable_model) }.encode_to_vec(),
    ))
}

pub async fn default_model(State(registry): State<TransportRegistry>) -> Result<Response<Body>> {
    let models = registry.store().models().await?;
    Ok(local_response(default_model_response(&models).encode_to_vec()))
}
...
pub async fn default_model_nudge(
    ...
    let models = registry.store().models().await?;
    Ok(local_response(default_model_nudge_response(&models).encode_to_vec()))
}

fn default_model_response(models: &[ModelConfig]) -> DefaultModelResponse {
    let model = models.first().map(|m| m.model_hash.clone()).unwrap_or_default();
...
fn default_model_nudge_response(models: &[ModelConfig]) -> DefaultModelNudgeDataResponse {
    DefaultModelNudgeDataResponse {
        ...
        models_with_no_default_switch: models.iter().map(|model| model.model_hash.clone()).collect(),
...
mod tests {
    ...
    fn cli_default_responses_use_the_local_model_hash() {
        ...
        let details = models.first().map(usable_model).expect("default model");
        assert_eq!(details.model_id, "local-model-hash");
